...
describe('<List />', () => {
  ...
  it('displays an anchor element', () => {
    const wrapper = mount(<List expansions={['a', 'b']} tag={TAG} />, options)
  ...
  it('allows the author to override the default conjunction', () => {
    ...
    const wrapper = mount(<List expansions={expansions} conjunction={conjunction} tag={TAG} />, options)
  ...
  it('allows the author to override the default separator', () => {
    const expansions = [['a', 'b', 'c'], 'd']
    const separator = '|'
    const wrapper = mount(<List expansions={expansions} separator={separator} tag={TAG} />, options)
    assert.equal('a|b| and c', wrapper.text())
  })
  ...
  it('leaves the last expansion linked if the `persistLast` prop is true', () => {
    const expansions = ['a', 'b', 'c']
    const wrapper = mount(<TestList persistLast expansions={expansions} tag={TAG} {...fakeStore} />)
    assert.equal(1, wrapper.find('a').length)
    wrapper.find('a').simulate('click')
    assert.equal(1, wrapper.find('a').length)
    wrapper.find('a').simulate('click')
  ...
  it('allows resetting an existing inventory property', () => {
    fakeStore.onUpdateInventory = sinon.spy()
    const expansions = ['a', ['b', 'c']]
    let wrapper = mount(<TestList persistLast expansions={expansions} tag={TAG} {...fakeStore} />)
    wrapper.find('a').simulate('click')
    assert(fakeStore.onUpdateInventory.calledWith(expansions[0], TAG))
    wrapper = mount(<TestList persistLast expansions={expansions} tag={TAG} {...fakeStore} currentExpansion={1} />)
